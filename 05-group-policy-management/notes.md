# Introduction to Group Policy Management and Domain Integration of another Windows PC (Desktop 2/User)

## Organizational Unit(OU) and Group Policy Management(GPO) in Windows 10 (Desktop 1/helpdesk)
 
### Create an OU in Desktop 1/helpdesk
Note: 
...
5. Repeat step 2 for IT(OU) and step 4 and helpdesk(User)

![Desktop-1 OU and User Creation](./screenshots/desktop1-account-creation-5.png)

### Group Policy Management Intro with Account Lockout Policy(GPO) in Desktop 1/helpdesk
Note:
- Group Policy - allows administrators to enforce specific settings and configurations on users and computers, like passwords.

- Computer Configuration -  Primary section of a Group Policy Object (GPO), these settings apply to the physical or virtual computer itself, regardless of who logs in.

- User Configuration -  Primary section of a Group Policy Object (GPO), these settings apply to the individual user account, regardless of which computer they are using on the network.  

1. Navigate to "Group Policy Management" -> Forest -> Domains -> Domain(Ex:lab.local) -> right-click "Default Domain Policy" -> Edit
...
- Make sure to hit "Apply" after any change made to a policy in order to take affect.
...
![Desktop Experience Setup](./screenshots/desktop-experience-setup-2.png) 
...
5. To Confirm domain integration is successful, try logging in with a a User account created in the Windows Server Domain 
